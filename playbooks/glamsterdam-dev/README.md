...
| `glamsterdam-devnet-6-eip7708-transfer-logs.yaml` | `glamsterdam-devnet-6-eip7708-transfer-logs` | ETH transfer log emission | EIP-7708 | Tests 1-4: (1) nonzero EOA→EOA emits log; (2) zero-value emits no log; (3) internal CALL-with-value emits log (deploys Forwarder); (4) CREATE-with-value emits log (deploys payable ValueReceiver) |
...
| `glamsterdam-devnet-6-eip7778-block-gas.yaml` | `glamsterdam-devnet-6-eip7778-block-gas` | Block gas accounting without refunds | EIP-7778 | Clears 8 storage slots (8×97920 gas refund); verifies block.gasUsed ≠ sum(receipt.gasUsed) — 2D accounting diverges when block contains refund txs |
| `glamsterdam-devnet-6-eip7928-bal-hash.yaml` | `glamsterdam-devnet-6-eip7928-bal-hash` | Block access list hash presence and consistency | EIP-7928 | Verifies blockAccessListHash is present in all blocks, non-empty in ALL blocks (including empty — EIP-4788 beacon root writes), and varies across blocks |
